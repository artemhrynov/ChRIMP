...
df = df_all[
    df_all.apply(
        ...
        and pd.isna(x["err_std"])
        and pd.isna(x["error_found"]),
        axis=1,
    )
].reset_index()
...
def tag_mapped_Hs_and_remap(smiles) -> tuple[dict, str]:
    ...
    for a in mol.GetAtoms():
        if a.GetSymbol() == "H" and (map_h := a.GetAtomMapNum()) > 0:
            ...
            if (
                other_atom.GetSymbol() != "H"
            ):  # Only annoying case would be H2, but we need to check for that
...
def map_remaining_heavy(smiles):
    ...
    already_used_indices = {
        int(match) for match in re.findall(idx_mapping_pattern, smiles)
    }
...
def remap_arrow(tup_str, remapping_dict, depth: int = 0):
    ...
    if isinstance(tup, int):
        ...
        if depth == 0:
            return str(
                tuple(
                    remap_arrow(str(sub_tup), remapping_dict, depth=depth + 1)
                    for sub_tup in tup
                )
            )
        else:
...
def mechsmiles_to_mapped_elementary(msmi_str):
    ...
    attacks_on_bonds = molecule_set.tag_attacks_on_bonds(
        [
            msmi.process_smiles_arrow(a, molecule_set.atom_map_dict)
            for a in msmi.smiles_arrows
        ]
    )
...
ds_df["mech_upsto_31k_format"] = ds_df["mech_smi_equ"].apply(
    lambda x: mechsmiles_to_mapped_elementary(x)
)

# %% Calculate the mapped elementary steps as well (Similar to FlowER)


def mechsmiles_to_mapped_elementary(msmi_str, removeHs=False):  # noqa: F811 (redefinition of the class defined above)
    msmi = MechSmiles(msmi_str)
...
ds_df["mapped_elem_w_hs"] = ds_df["mech_smi_equ"].apply(
    lambda x: mechsmiles_to_mapped_elementary(x)
)
ds_df["mapped_elem_wo_hs"] = ds_df["mech_smi_equ"].apply(
    lambda x: mechsmiles_to_mapped_elementary(x, removeHs=True)
)
...
ds_df["mech_smi_min_len"] = ds_df["mech_smi_min"].apply(
    lambda x: len(str(x.replace(" ", "")))
)
ds_df["mech_smi_equ_len"] = ds_df["mech_smi_equ"].apply(
    lambda x: len(str(x.replace(" ", "")))
)
...
ds_df["mech_upsto_31k_format_len"] = ds_df["mech_upsto_31k_format"].apply(
    lambda x: len(str(x))
)
...
plot_data_melted = plot_data.melt(
    var_name="Representation", value_name="Character Length"
)
...
plot_data_renamed["Representation"] = plot_data_renamed["Representation"].map(
    label_mapping
)
...
plt.savefig(
    os.path.join(
        "data", "figures", "paper_experiments", "comparison_char_efficiency.svg"
    )
)
plt.show()
...
plt.savefig(
    os.path.join(
        "data", "figures", "paper_experiments", "comparison_char_efficiency_zoom.svg"
    )
)
plt.show()
...
print(
    f"  mech_smi_min: {(mean_values['mech_smi_min'] / mean_values['pmechdb_str'] * 100):.2f}%"
)
print(
    f"  mech_smi_equ: {(mean_values['mech_smi_equ'] / mean_values['pmechdb_str'] * 100):.2f}%"
)
print(
    f"  mapped_elem_w_hs: {(mean_values['mapped_elem_w_hs'] / mean_values['pmechdb_str'] * 100):.2f}%"
)
print(
    f"  mapped_elem_wo_hs: {(mean_values['mapped_elem_wo_hs'] / mean_values['pmechdb_str'] * 100):.2f}%"
)
