...
df_add_info = pd.read_parquet(f"data/evaluations/model_{model_n}/test_retro_questions.parquet")

# Filter because of truncation
sanity_checked_input = df['input'].unique()
df_add_info = df_add_info[df_add_info['input'].isin(sanity_checked_input)].reset_index(drop=True)
...
while changes_this_loop:
    ...
    for i in range(len(df_add_info)):
        if i >= len(df_best):
            ...
            changes_this_loop = True
            break
        if df_add_info.loc[i]['output'].replace(' ', '') != df_best.loc[i]['gold_output'].replace('[eos]', ''):
...
if not len(df) == len(df_add_info)*top_k:
    ...
    print(f"Correct beam is most probably {len(df['beam_index'].unique())} == {len(df)/len(df_add_info)}")
    print(f"If these two numbers are not equal or non-integer, there is an error")
...
df['step_idx_retro'] = [df_add_info['metadata'].values[i//top_k]['step_idx_retro'] for i in range(len(df))]
...
if len_problem_output >0:
    print(f"{Fore.RED}Careful, had to remove {len_problem_output} outputs because then don't end with [eos]")
df = df[df['gold_output'] != "[eos]"]
print(len(df))
df['ready_for_search'] = ((df['product_stable']==1) & (df['msmi_reac_correct']==1)).astype(float)

filtered_df = df[(df['ground_truth_equiv'] == True) & (df['ready_for_search'] != 1)]
...
mask = (
    df.groupby(['input', 'gold_output'])['ready_for_search']
      .transform(lambda s: (s != 1).all())          # True if no 1-s in the group
)
...
sns.scatterplot(x=x_axis,y=[x*100 for x in perc_results], label="Normal order", zorder=2)
sns.lineplot(x=x_axis,y=[x*100 for x in perc_results_prechecked], zorder=1)
sns.scatterplot(x=x_axis,y=[x*100 for x in perc_results_prechecked], label=f"Heuristic reorder (among top-{top_k})", zorder=1)

sns.scatterplot(x=[top_reaching_index+1], y=[max(perc_results_prechecked)*100], color=sns.color_palette()[1], marker="*", s=300, label=f"Max reached (@ top-{top_reaching_index+1})") #color="#eec611"
...
sns.lineplot(x=x_axis,y=[(x_i+y_i)*100 for x_i, y_i in zip(percentage_usable_for_search,percentage_legal_but_not_correct_reac)], zorder=1, ls='--')
sns.scatterplot(x=x_axis,y=[(x_i+y_i)*100 for x_i, y_i in zip(percentage_usable_for_search,percentage_legal_but_not_correct_reac)], label="Sum of legal MechSMILES")

sns.lineplot(x=x_axis,y=[x*100 for x in percentage_usable_for_search], zorder=1)
sns.scatterplot(x=x_axis,y=[x*100 for x in percentage_usable_for_search], label="MechSMILES legal + correct reac")

sns.lineplot(x=x_axis,y=[x*100 for x in percentage_legal_but_not_correct_reac], zorder=1)
sns.scatterplot(x=x_axis,y=[x*100 for x in percentage_legal_but_not_correct_reac], label="MechSMILES legal + incorrect reac")
...
plt.fill_between(
    ...
    color="gray", alpha=0.3, label="Gap between exact and equiv"
)

sns.lineplot(x=x_axis,y=[x*100 for x in percentage_usable_for_search], zorder=1)
sns.scatterplot(x=x_axis,y=[x*100 for x in percentage_usable_for_search], label="Correct reac. (stable prod.)")

sns.lineplot(x=x_axis,y=[x*100 for x in equiv_correct_answer], zorder=1)
sns.scatterplot(x=x_axis,y=[x*100 for x in equiv_correct_answer], label="Equiv. to ground-truth")

sns.lineplot(x=x_axis,y=[x*100 for x in exact_correct_answer], zorder=1)
sns.scatterplot(x=x_axis,y=[x*100 for x in exact_correct_answer], label="Exact ground-truth")
...
if results_folder is not None:
    plt.savefig(os.path.join(results_folder, "svg", "top_kth_correct_reac_equiv_exact.svg"))
...
# %% Analyze templates of each elem steps
#from rules import get_rxn_smarts_template
#from rules.utils.templates import safe_get_rxn_smarts_template
#from rdkit.Chem import MolFromSmiles, MolToSmiles
#
#all_gold_out = df['gold_output'].unique()
## Remove and stop at the 1st '[eos]'
#all_gold_out = [gold_out.split('[eos]')[0] for gold_out in all_gold_out]
#
#def rules_template_from_mechsmiles(msmi):
#    reac = msmi.ms.rdkit_mol
#    prod = msmi.ms_prod.rdkit_mol
#
#    for i,a in enumerate(reac.GetAtoms()):
#        a.SetAtomMapNum(i+1)
#
#    for i,a in enumerate(prod.GetAtoms()):
#        a.SetAtomMapNum(i+1)
#
#    mapped_rxn = f"{MolToSmiles(reac)}>>{MolToSmiles(prod)}"
#
#    try:
#        mapped_smarts, unmapped_smarts = get_rxn_smarts_template(mapped_rxn, with_decorators=False)
#        
#        if unmapped_smarts == '>>':
#            raise ValueError(f'Non-reaction with MechSMILES {msmi.value}')
#
...
## %% Observe reaction templates
#results_dict = {}
#
#from concurrent.futures import ProcessPoolExecutor
#inputs = [gold_out for gold_out in all_gold_out]
#
#def worker(s):
#    try:
#        return rules_template_from_mechsmiles(MechSmiles(s))
#    except:
#        return "Error"
#
#max_workers = 14
#with ProcessPoolExecutor(max_workers=max_workers) as ex:
#    all_res = list(tqdm(ex.map(worker, inputs, chunksize=64), total=len(inputs)))
#    results_dict = dict(zip(all_gold_out, all_res))
#
##for gold_out in tqdm(all_gold_out):
##    results_dict[f"{gold_out}"] = rules_template_from_mechsmiles(MechSmiles(gold_out))
#
## %% Observe these templates
#
#df['ground_truth_rules_template'] = df['gold_output'].apply(lambda x: results_dict[x])
#df_top1 = df[df['beam_index'] == 0]
##df_prechecked['ground_truth_rules_template'] = df_prechecked['gold_output'].apply(lambda x: results_dict[x])
#counts_elem_templates_in_eval = df_top1['ground_truth_rules_template'].value_counts()
#counts_elem_templates_in_eval[:10]
...
df_heuristic_top1_or_top1 = ( # 1st heuristic or top-1 is no answer in top-k passes the filters
    df.sort_values(
        ["ready_for_search", "beam_index"], # ensure it is sorted (But ready for search first)
...
df_heuristic_top1_correct = df_heuristic_top1[df_heuristic_top1['ground_truth_equiv']==1]
df_heuristic_top1_incorrect = df_heuristic_top1[df_heuristic_top1['ground_truth_equiv']!=1]
...
if multiple_length_routes:
    ...
    for i in range(0, int(df_heuristic_top1_or_top1['step_idx_retro'].max())):
        print(i)
        df_heuristic_top1_or_top1_step = df_heuristic_top1_or_top1[df_heuristic_top1_or_top1['step_idx_retro'] == i]
        df_heuristic_topn_or_topn_step = df_heuristic_topn_or_topn[df_heuristic_topn_or_topn['step_idx_retro'] == i]
    ...
    if results_folder is not None:
        plt.savefig(os.path.join(results_folder, "svg", "distribution_distance_from_prod.svg"))
...
if multiple_length_routes:
    ...
    success_rates_heur_or_top1 = [s*100 for s in distance_to_goal_success_rate_heur_or_top.values()]
    success_rates_heur_or_topn = [s*100 for s in distance_to_goal_success_rate_heur_or_topn.values()]
    ...
    sns.lineplot(x=steps[:lim], y=success_rates_heur_or_topn[:lim], ax=ax2, ls = '-', marker='o', color=scatter_color, label=f'Top-{n}')
    sns.lineplot(x=steps[:lim], y=success_rates_heur_or_top1[:lim], ax=ax2, ls='-',  marker='o', color=scatter_color2, label='Top-1')
    ...
    if results_folder is not None:
        plt.savefig(os.path.join(results_folder, "svg", "success_rate_vs_distance_to_goal_heur_or_top_n.svg"))
        plt.savefig(os.path.join(results_folder, "success_rate_vs_distance_to_goal_heur_or_topn.png"))
        plt.clf()
        plt.close()
    else:
        plt.show()
...
df_add_info['retro_step_idx'] = df_add_info['metadata'].apply(lambda x: x['step_idx_retro'])
...
if not os.path.isfile(f"long_computation_data_model_{model_n}.pkl"):
        ...
        for index in tqdm(all_test_indices[:]):
            ...
            for input in sub_df['input'].values:
                ...
                if len(df_beam_equiv_gt) == 0:
                    ...
                    min_beam_width_necessary = max(min_beam_width_necessary, min_beam_width)

            if successful:
                num_success_rxns_per_length[length-1].append(int(min_beam_width_necessary))
    ...
    with open(f"long_computation_data_model_{model_n}.pkl", 'rb') as f:
        ...
        num_rxns_per_length, num_success_rxns_per_length, total_num_rxns, total_success_rxns = loaded_data
...
aggr_num_success_rxns_per_length = (
    num_success_rxns_per_length[:max_interesting_length].copy()
    + [sum(num_success_rxns_per_length[max_interesting_length:], [])]
)

# Prepare data for plotting
data = []
for threshold in thresholds:
    for length_idx, (total, success_list) in enumerate(zip(aggr_num_rxns_per_length, aggr_num_success_rxns_per_length)):
...
sns.lineplot(data=df_plot, x='Length', y='Percentage', hue='Threshold', marker='o', palette=palette)
...
plt.xticks(
    ...
    labels = [str(x) + ('+' if x > max_interesting_length else '') for x in range(1, len(aggr_num_rxns_per_length)+1)],
)
...
df_melted = df_flower_paper.melt(id_vars='Top-k', var_name='Method', value_name='Accuracy')
...
df_melted = data_mech_uspto_31k_paper.melt(id_vars='Top-k', var_name='Method', value_name='Accuracy')
...
df_melted = df_reactron_paper.melt(id_vars='Top-k', var_name='Method', value_name='Accuracy')
df_melted = df_melted[df_melted['Top-k']<=10]
...
#sns.lineplot(data=df_plot, x='Beam size', y='Percentage', marker='o')
plt.axhline(y=95.9, color=sns.color_palette('magma', n_colors=4)[0], 
            linestyle='--', alpha=0.5, linewidth=1.5, xmin=0.2, xmax=1.0)
plt.axhline(y=56.7, color=sns.color_palette('magma', n_colors=4)[1], 
            linestyle='--', alpha=0.5, linewidth=1.5, xmin=0.2, xmax=1.0)
plt.axhline(y=43.5, color=sns.color_palette('magma', n_colors=4)[2], 
            linestyle='--', alpha=0.5, linewidth=1.5, xmin=0.2, xmax=1.0)
...
if multiple_length_routes:
    ...
    # Scatterplot (left y-axis)
    sns.lineplot(x=steps, y=success_rates_topn, ax=ax2, marker='o', color=scatter_color2, ls='--', label=f'top-{n}')

    sns.lineplot(x=steps, y=success_rates_top1, ax=ax2, marker='o', color=scatter_color, ls='--', label='top-1')
    ...
    if results_folder is not None:
        plt.savefig(os.path.join(results_folder, "svg", "success_rate_vs_distance_to_goal_topn.svg"))
        plt.savefig(os.path.join(results_folder, "success_rate_vs_distance_to_goal_topn.png"))
        plt.clf()
        plt.close()
    else:
        plt.show()
...
df_no_heur_nothing = df_no_heur_top1_legal_reac[(df_no_heur_top1_legal_reac['msmi_reac_correct']!=1) & ((df_no_heur_top1_legal_reac['product_stable']==0))]
df_no_heur_not_correct_reac = df_no_heur_top1_legal_reac[(df_no_heur_top1_legal_reac['msmi_reac_correct']!=1) & ((df_no_heur_top1_legal_reac['product_stable']==1))]
df_no_heur_prod_arrow_push_not_legal = df_no_heur_top1_legal_reac[(df_no_heur_top1_legal_reac['msmi_reac_correct']==1) & ((df_no_heur_top1_legal_reac['product_stable']==0))]

no_heur_a, label_a = len(df_no_heur_not_correct_reac) / num_questions, f'Incorrect reac. (stable prod.)'
no_heur_b, label_b = len(df_no_heur_illegal_smiles)/ num_questions, f"Illegal SMILES"
no_heur_c, label_c = len(df_no_heur_nothing) / num_questions, f'Incorrect reac. (unstable prod.)'
no_heur_d, label_d = len(df_no_heur_prod_arrow_push_not_legal) / num_questions, f'Correct reac. with illegal arrow-flow'
...
for i, (h, b, cat) in enumerate(zip(heights, bottoms, categories)):
    ax.text(x[2], b + h/2, f"{h:.2%}" if h>= 0.0005 else '', ha='center', va='center', fontsize=16)
...
# %% Look at what has been proposed for incorrect that found a heuristic answer

# First predict them
unique_pred_outputs = df_heuristic_top1_incorrect['generated_text'].unique()
unique_pred_outputs = [pred.split('[eos]')[0] for pred in unique_pred_outputs]

results_dict = {}
for pred_output in tqdm(unique_pred_outputs[:]):
    #print(f"{pred_output = }")

    try:
        results_dict[pred_output+'[eos]'] = rules_template_from_mechsmiles(MechSmiles(pred_output))
    except Exception as e:
        print(f"Error processing {pred_output}: {e}")

df_heuristic_top1_incorrect['pred_rules_template'] = df_heuristic_top1_incorrect['generated_text'].apply(lambda x: results_dict.get(x, "Error"))

# %% Show some examples
from Levenshtein import distance as lev_dist

counts = df_heuristic_top1_incorrect[['ground_truth_rules_template','pred_rules_template']].value_counts()

def compare_two_mechsmiles(msmi1, msmi2, title=None, show_plot=True, string_distance=True):
    """
    ...
    print(f"MechSMILES:\n{msmi1.value}\n{msmi2.value}")
    print(f"Reactants:\n{msmi1.smiles}\n{msmi2.smiles}")
    plt.figtext(0.5, 0.02, f"Lehvenstein between\nMechSMILES: {lev_dist(msmi1.value, msmi2.value)}\nReactants: {lev_dist(msmi1.smiles, msmi2.smiles)}", ha="center", fontsize=10)
...
tuples_gt_pred = [(re.sub(r'\[eos\]', '', a),re.sub(r'\[eos\]', '', b))  for a,b in tuples_gt_pred]

for i in range(20):
    print()
    print(f"{i = }, distance_to_goal = {df_no_heur_not_correct_reac['step_idx_retro'].values[i]+1}")
...
for i in range(20):
    print()
    print(f"{i = }, distance_to_goal = {df_heuristic_top1_incorrect['step_idx_retro'].values[i]+1}")
...
print(f'"{sub_df["metadata"].values[0]["elem_reac_equ"]}>>{sub_df["metadata"].values[-1]["elem_prod_equ"]}"')

print(sub_df["metadata"].values[0]["elem_reac_equ"]+ '>>' + '>>'.join(f"{x['elem_prod_equ']}" for x in sub_df['metadata'].values))
