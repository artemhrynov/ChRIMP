...
def candidate_score(
    candidate_mech_smi: str,
    target_counter: Counter[str],
    target_counter_uncharged: Counter[str],
) -> tuple[int, str | None]:
    try:
    ...
    if candidate_counter is None:
        return 0, candidate_product

    if candidate_counter == target_counter:
        return 4, candidate_product
    if counter_contains(candidate_counter, target_counter):
        return 3, candidate_product

    candidate_counter_uncharged = canonical_component_counter(
        candidate_product, uncharge=True
    )
    if candidate_counter_uncharged is None:
        return 0, candidate_product

    if candidate_counter_uncharged == target_counter_uncharged:
        return 2, candidate_product
    if counter_contains(candidate_counter_uncharged, target_counter_uncharged):
        return 1, candidate_product
...
def infer_th_mech_smi(mech_smi: str, target_smiles: str) -> tuple[str, str]:
    ...
    target_counter_uncharged = canonical_component_counter(target_smiles, uncharge=True)
    if target_counter_uncharged is None:
        return mech_smi, "target_canonicalization_failed"
    ...
    for modes in product(mode_options, repeat=len(events)):
        ...
        ranking = (score, preference, compactness, candidate, modes)
    ...
    _, _, _, best_candidate, best_modes = best
    return best_candidate, "added_" + "_".join(best_modes)
