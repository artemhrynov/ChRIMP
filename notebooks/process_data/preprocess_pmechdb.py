...
from chrimp.world.molecule_set import MoleculeSet, RadicalAtomException,  ReusedVirtualTSException
...
for split in splits:
    ...
    df_raw = pd.read_csv(
        os.path.join("data", "pmechdb", "mc_train_fold0", "reformatted", f"{split}.txt"),
        names=["smirks", "arrow_code", "source", "sink"]
    )

    df_raw["pmechdb_str"] = df_raw.apply(lambda x: f"{x['smirks']} {x['arrow_code']}", axis=1)
    df_raw["split"] = split
...
print(f"And non-trivial              : {len_df_digit_non_trivial:_} ({len_df_digit_non_trivial/len_df_all:.2%} of previous)")
...
df.to_csv(os.path.join("data", "pmechdb", f"semi_final.csv"), index=None)


# %%
df_correct_filtered = df[df.apply(lambda x: x['err_std'] == '' and x['error_found'] == '', axis = 1)]
df_correct_filtered = df_correct_filtered.drop(columns=["mech_smi_raw", "error_found", 'err_std', 'smirks', 'arrow_code', 'pmechdb_str', 'source', 'sink'])
df_correct_filtered.columns


df_correct_filtered.to_csv(os.path.join("data", "preprocessed", "pmechdb_split_0.csv"), index=False)
