...
df_raw = pd.read_csv(os.path.join("data", "flower_nature", "original", "data", "flower_new_dataset", f"{split}.txt"), header=None, sep='|', names=['elem_rxn', 'rxn_idx'])
...
print(f"Len of df with digits indices: {len_df_digit:_} ({len_df_digit/len_df_all:.2%} of previous)")
print(f"And non-trivial              : {len_df_digit_non_trivial:_} ({len_df_digit_non_trivial/len_df_digit:.2%} of previous)")
...
def mapped_elem_to_mechsmiles(mapped_rxn, verbose=False):
    ...
    for i, atom in enumerate(reac_mol.GetAtoms()):
        ...
        for bond in a.GetBonds():
            ...
            bonds_reac[map_dict[a.GetAtomMapNum()], map_dict[b.GetOtherAtom(a).GetAtomMapNum()]] = int(deg)
    ...
    for i, atom in enumerate(prod_mol.GetAtoms()):
        ...
        for bond in a.GetBonds():
            ...
            if deg != int(deg):
                return "", "Aromatic bond detected"
                raise ValueError(f"Aromatic bond detected with {mapped_rxn}")
            bonds_prod[map_dict[a.GetAtomMapNum()], map_dict[b.GetOtherAtom(a).GetAtomMapNum()]] = int(deg)
    ...
    nodes_plus_one = [node for node, data in G.nodes(data=True) if data['charge_diff'] == +1]
    nodes_minus_one = [node for node, data in G.nodes(data=True) if data['charge_diff'] == -1]
    ...
    if path_found:
        ...
        for i, ef in enumerate(edge_features):
            primitive_arrow = ('i' if ef == -1 else 'a', node_features[i], node_features[i+1])
        ...
        while len(primitive_arrows)>0:
            ...
            if len(primitive_arrows) > 1 and primitive_arrows[0][0] == 'i' and primitive_arrows[1][0] == 'a' and  primitive_arrows[0][2] == primitive_arrows[1][1]:
        ...
        if verbose:
            print("Condition not met: need exactly one node with charge_diff = -1 and one with charge_diff = +1 or zero of each")
...
df.to_csv(os.path.join("data", "flower_nature", "flower_new", f"{split}_intermediate.csv"), index=None)
...
def check_same_prod(rxn, msmi_string):
    if not pd.isna(msmi_string):
        try:
            ...
            same = Chem.MolToSmiles(mol) == Chem.MolToSmiles(Chem.MolFromSmiles(msmi.ms_prod.can_smiles))
            #if not same:
            return same, ""
...
df.to_csv(os.path.join("data", "flower_nature", "flower_new", f"{split}.csv"), index=None)
...
df_train = pd.read_csv(os.path.join("data", "flower_nature", "flower_new", f"train.csv"))
df_val = pd.read_csv(os.path.join("data", "flower_nature", "flower_new", f"val.csv"))
df_test = pd.read_csv(os.path.join("data", "flower_nature", "flower_new", f"test.csv"))
...
def standardize_msmi(msmi_str):
    return (MechSmiles(msmi_str).standard_value, "") if not pd.isna(msmi_str) else ("", "")
...
df.to_csv(os.path.join("data", "flower_nature", "flower_new", f"semi_final.csv"), index=None)


# %%

df = pd.read_csv(os.path.join("data", "flower_nature", "flower_new", f"semi_final.csv"))
...
df_correct_filtered = df_correct.groupby('rxn_idx').filter(has_complete_distance_sequence)


# %%


df_correct_filtered = df_correct_filtered.drop(columns=["mech_smi_raw", "error_found", 'error_same_prod', 'err_std', 'elem_rxn', 'correct_prod'])
df_correct_filtered.columns


df_correct_filtered.to_csv(os.path.join("data", "preprocessed", "flower_new_dataset.csv"), index=False)
...
df_correct_filtered = pd.read_csv(os.path.join("data", "preprocessed", "flower_new_dataset.csv"))
