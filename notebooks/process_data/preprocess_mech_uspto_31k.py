...
from chrimp.world.molecule_set import BondNotFoundError, ReusedVirtualTSException, MoleculeSet
...
df[["reac_smiles", "cond_smiles", "prod_smiles"]] = df["updated_reaction"].str.split('>', expand=True)

# Remove trivial duplicates
total_len = len(df)
df_no_dupl = df.drop_duplicates(subset=['reac_smiles', 'prod_smiles', 'mechanistic_label'])
len_no_dupl = len(df_no_dupl)

# Remove duplicates (trivial)
df_rxn = df_no_dupl[df_no_dupl.apply(lambda x: x['reac_smiles'] != x['prod_smiles'], axis=1)]
...
def get_n_indices_not_in_given_set(n, given_set):
    ...
    assert n < MAX_ITER, f"n is too big, surpasses {MAX_ITER = }, you should probably not use that function for such big n"
    for _ in range(MAX_ITER):
        if not i in given_set:
...
def unmap_linked_hs(smiles: str):
    ...
    for a in mol.GetAtoms():
        if a.GetSymbol() == 'H' and a.GetAtomMapNum() > 0:
            ...
            if not all([b.GetOtherAtom(a).GetSymbol() == 'H']): # At least linked to one heavy atom
                a.SetAtomMapNum(0)
...
def expand_one_h(smiles: str, x: int, y: int, first_run:bool=True) -> str:
    ...
    Chem.SanitizeMol(mol, Chem.SanitizeFlags.SANITIZE_ALL ^ Chem.SanitizeFlags.SANITIZE_ADJUSTHS ^ Chem.SanitizeFlags.SANITIZE_KEKULIZE)
...
def verify_extract_uspto_31k(mapped_rxn_reac, mapped_rxn_prod, arrow_sequence, verbose=False):
    ...
    all_indices_arrows = set().union(*[get_indices_from_tuple(literal_eval(a)) for a in arrows_mechsmiles])
    ...
    new_indices_implicit_hs = get_n_indices_not_in_given_set(len(indices_implicit_hs), all_indices_reac)
    ...
    while len(arrows_mechsmiles) > 0:
        ...
        while j < len(arrows_mechsmiles):
            ...
            mapped_rxn_reac = unmap_given_indices(mapped_rxn_reac, new_indices_implicit_hs)
            for old_idx in all_floats:
                if verbose:
                    print(f"{Fore.GREEN}{all_floats}{Fore.RESET}")
                old_index_heavy, sub_index = int(str(old_idx).split('.')[0]), int(str(old_idx).split('.')[1])
                if sub_index != 1:
                    print(f"Sub-index not 1 in:\n{mapped_rxn_reac = }\n{picked_arrows = }")
                mapped_rxn_reac = expand_one_h(mapped_rxn_reac, old_index_heavy, dict_of_replacements[old_idx], first_run=first_run)
                first_run = False


            picked_arrows = [str(rewrite_new_tuple(a, dict_of_replacements)) for a in picked_arrows]
            ...
            try:
                ...
                mapped_rxn_reac = unmap_given_indices(ms_prod_found.mapped_smiles, new_indices_implicit_hs)
...
df_rxn[df_rxn['reac_smiles'] == "[OH:101][CH:1]([CH2:2][CH3:3])[c:4]1[n:5][n:6]2[cH:7][cH:8][cH:9][c:10]2[c:11](=[O:12])[n:13]1[CH2:14][c:15]1[cH:16][cH:17][cH:18][cH:19][cH:20]1.[CH3:21][C:22]([CH3:23])([CH3:24])[O:25][C:26](=[O:27])[NH:28][CH2:29][CH2:30][CH2:31][NH2:32].[O:301]=[S:302]([Cl:303])(=O)c1ccc(cc1)C"]['mechanistic_label'].values[0]
...
a, b = verify_extract_uspto_31k(
    ...
    [(101, 302), ([302, 301], 301), (301, [301, 302]), ([302, 303], 303), (303, 101.1), ([101.1, 101], 101), (32, 1), ([1, 101], 101), (101, 32.1), ([32.1, 32], 32)],
    verbose=True,
)
print('\n'.join([MechSmiles(f"{a_i}|{b_i}").standard_value for a_i, b_i in zip(a, b)]))
print('\n'.join([MechSmiles(f"{a_i}|{b_i}").value for a_i, b_i in zip(a, b)]))
...
a, b = verify_extract_uspto_31k(
    ...
    [(5, 301), (6, 4), ([4, 5], 5), (101, 6.1), ([6.1, 6], 6), (5, [5, 4]), ([4, 101], 101), ([301, 5], 5)],
    verbose=True,
)
print(a)
print(b)
...
a, b = verify_extract_uspto_31k(
    ...
    [([102, 16], 1), ([1, 2], 102), ([102, 2], 301), (101, 302), ([301, 2], [2, 1]), ([1, 101], 101)],
    verbose=True,
)
print(a)
print(b)
...
a, b = verify_extract_uspto_31k(
    ...
    [(301, 1), ([1, 102], 102), (102, 101.1), ([101.1, 101], 101), (101, 301), ([301, 1], 1), (1, 2), ([2, 101], [101, 301])],
)
print(a)
print(b)

df_rxn[df_rxn['reac_smiles'] == "[Br:101][CH2:1][c:2]1[n:3][c:4]2[c:5]([NH2:6])[n:7][cH:8][n:9][c:10]2[n:11]1[CH2:12][CH2:13][c:14]1[cH:15][cH:16][cH:17][cH:18][cH:19]1.[CH3:20][CH2:21][O:22][P:23](=[O:24])([CH2:25][OH:26])[O:27][CH2:28][CH3:29].[H-:301].[Na+]"]['prod_smiles'].values[0]
...
def _worker(args):
    ...
    try:
        ...
        arrows = (literal_eval(mechanistic_label)
                  if isinstance(mechanistic_label, str) else mechanistic_label)
...
rows = list(
    df_rxn[["reac_smiles", "prod_smiles", "mechanistic_label", "row_number"]]
...
# %%
data = []
for idx, (r_list,a_list, m_r, m_p, row_idx) in enumerate(zip(all_stable_lists, all_arrow_flows, all_mapped_reac, all_mapped_prod, all_row_indices)):
    for sub_idx, (r,a) in enumerate(zip(r_list, a_list)):
        data.append([f"{r}|{a}", idx, len(r_list), len(r_list)-sub_idx-1, m_r, m_p, f"rxn_{row_idx}"])

df = pd.DataFrame(data, columns=["mech_smi", "single_step_idx", "single_step_length", "step_idx_retro", "single_step_original_reac", "single_step_original_prod", "source"])
df.to_csv(os.path.join("data", "uspto-31k", "first_mechsmiles_extraction.csv"), index=False)
...
plt.title(f"Distribution of {len(lengths)} {len(lengths)/len(df_rxn):.2%} single steps, length mean = {avg_len:.4f}")
...
df['mech_smi_tuple'] = df['mech_smi_raw'].parallel_apply(lambda x: safe_msmi_standardize(x))

df[['mech_smi', 'could_canonicalize']] = pd.DataFrame(df['mech_smi_tuple'].tolist(), index=df.index)
df = df.drop(columns=['mech_smi_tuple'])
df.to_csv(os.path.join("data", "uspto-31k", "first_mechsmiles_extraction_intermediate.csv"), index=False)


# %%
df = pd.read_csv(os.path.join("data", "uspto-31k", "first_mechsmiles_extraction_intermediate.csv"))

df['could_canonicalize'].value_counts()
df = df.drop(columns=['could_canonicalize'])

df['reac_elem_step'] = df['mech_smi'].parallel_apply(lambda x: MechSmiles(x).ms.can_smiles)
...
df['prod_elem_step'] = df.parallel_apply(lambda x: safe_prod(x['mech_smi'], x['mech_smi_raw']), axis=1)
