...
df_pre["step_idx_forward"] = df_pre.parallel_apply(
    lambda x: x["rxn_length"] - x["step_idx_retro"] - 1, axis=1
)
...
df_last_elem_step["atom_counter"] = df_last_elem_step["mech_smi"].parallel_apply(
    lambda x: Counter([a.GetSymbol() for a in MechSmiles(x).ms.rdkit_mol.GetAtoms()])
)
dict_atoms = dict(
    zip(df_last_elem_step["rxn_idx"].values, df_last_elem_step["atom_counter"].values)
)
df_pre["equ_with_last_step"] = df_pre.parallel_apply(
    lambda x: Counter(
        [a.GetSymbol() for a in MechSmiles(x["mech_smi"]).ms.rdkit_mol.GetAtoms()]
    )
    == dict_atoms[x["rxn_idx"]],
    axis=1,
)
...
if already_equilibrated:
    ...
    df_non_equ = df_pre[~df_pre["equ_with_last_step"]]
    len(df_non_equ)
    df_non_equ[["rxn_idx", "rxn_length", "step_idx_retro", "equ_with_last_step"]]
    raise NotImplementedError(
        "Implemenatation has been done for pmechdb, needs to be imported here"
    )
...
df_pre["elem_reac_prod_equ_tuple"] = df_pre["mech_smi_equ"].parallel_apply(
    lambda x: can_reac_prod_from_msmi(MechSmiles(x))
)
df_pre[["elem_reac_equ", "elem_prod_equ"]] = pd.DataFrame(
    df_pre["elem_reac_prod_equ_tuple"].tolist(), index=df_pre.index
)
...
df_pre["mech_smi_min"] = df_pre["mech_smi_equ"].parallel_apply(
    lambda x: hide_cond_get_value(MechSmiles(x))
)

df_pre["elem_reac_prod_min_tuple"] = df_pre["mech_smi_min"].parallel_apply(
    lambda x: can_reac_prod_from_msmi(MechSmiles(x))
)
df_pre[["elem_reac_min", "elem_prod_min"]] = pd.DataFrame(
    df_pre["elem_reac_prod_min_tuple"].tolist(), index=df_pre.index
)
...
df_last_elem_step = df_pre[df_pre["step_idx_retro"] == 0]
dict_last_equ = dict(
    zip(df_last_elem_step["rxn_idx"].values, df_last_elem_step["elem_prod_equ"].values)
)
dict_last_min = dict(
    zip(df_last_elem_step["rxn_idx"].values, df_last_elem_step["elem_prod_min"].values)
)
...
if "split" in df_pre.columns:
    ...
    val_indices = all_rxn_indices[
        int(num_single_steps * (tvt_split[0])) : int(
            num_single_steps * (tvt_split[0] + tvt_split[1])
        )
    ]
    test_indices = all_rxn_indices[
        int(num_single_steps * (tvt_split[0] + tvt_split[1])) :
    ]
