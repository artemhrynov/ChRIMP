...
from chrimp.agent.supervised_evaluate import (
    evaluation_metrics_w_groundtruth,
    answers_dict,
)
...
def _worker_function(
    ...
    try:
        ...
        for format_type in input_format.split(";"):
            try:
                formatted_input = human_to_input(
                    human_input, format_type, already_canonicalized=False
                )
                entry["formatted_inputs"][format_type] = formatted_input
            except Exception as e:
                print(
                    f"Warning: Failed to format input for format '{format_type}' at row {idx}: {e}"
                )
...
class NLPAgent(ABC):
    ...
    def process_dataset_for_nlp(
        self,
        dataset,
        task,
        input_format,
        code_mechsmiles_output: str = "min",
        ...
        for idx, row in enumerate(dataset):
            try:
                ...
                if with_metadata:
                    entry["metadata"] = {
                        c: row[c] for c in dataset.column_names
                    }  # Every row that is not already encoded above

                for format_type in input_format.split(";"):
                    try:
                        formatted_input = self.human_to_input(
                            human_input, format_type, already_canonicalized=False
                        )
                        entry["formatted_inputs"][format_type] = formatted_input
                    except Exception as e:
        ...
        if with_metadata:
            return Dataset.from_dict(
                {"input": inputs, "output": outputs, "metadata": metadatas}
            )
        else:
            return Dataset.from_dict({"input": inputs, "output": outputs})

    def process_dataset_for_nlp_parallel(
        self,
        dataset,
        task,
        input_format,
    ...
    def supervised_train(
        ...
        batch_size: int = 64,
        dataset_name: str
        | None = None,  # If None, we'll take the string after last '/'
    ):
        if self.tokenizer is None or self.model is None:
            raise ValueError(
                f"Tokenizer ({self.tokenizer}) or model ({self.model}) cannot be None, please initialize or load them before training"
            )
        ...
        if self.inference_input_format not in train_input_format_list:
            print(
                f"{Fore.LIGHTYELLOW_EX}{self.inference_input_format = } is not seen during training ({train_input_format = }). This might lead to large performance loss.{Fore.RESET}"
            )
        ...
        tokenized_train_dataset, tokenized_val_dataset = (
            self.import_process_tokenize_dataset(
        ...
        train_args = TrainingArguments(
            ...
            output_dir=os.path.join(
                ...
                "checkpoints_hf_train"
                if run_int is None
                else f"checkpoints_hf_train_debug_{run_int}",
            ),
        )
    ...
    def supervised_eval(
        ...
        if generate_only and score_only:
            raise ValueError(
                "Both generate_only and score_only are True, this behaviour is ambiguous"
            )

        if (generate_only or score_only) and intermediate_pickle_path is None:
            raise ValueError(
                "One must give an intermediate_pickle_path to do only generation or only scoring"
            )

        if not score_only:
            if self.tokenizer is None or self.model is None:
                raise ValueError(
                    f"Tokenizer ({self.tokenizer}) or model ({self.model}) cannot be None, please initialize or load them before evaluating"
                )
            ...
            time1 = time()
            print(
                f"{Fore.GREEN}Time to import, process, tokenize: {time1-time0:>6.2f}s{Fore.RESET}"
            )
            ...
            time2 = time()
            print(
                f"{Fore.GREEN}Time to sanitiy check: {time2-time1:>6.2f}s{Fore.RESET}"
            )
            ...
            def collate_fn(batch):
                ...
                if self.is_decoder_only:
                    ...
                    input_ids = [
                        torch.tensor(ex["input_ids"][: ex["mech_idx"]]) for ex in batch
                    ]
                    attention_masks = [
                        torch.tensor(ex["attention_mask"][: ex["mech_idx"]])
                        for ex in batch
                    ]
                else:
                    ...
                    attention_masks = [
                        torch.tensor(
                            ex.get("attention_mask", [1] * len(ex["input_ids"]))
                        )
                        for ex in batch
                    ]
                ...
                if self.is_decoder_only:
                    decoded_labels = [
                        ex["output_text"].replace(" ", "") for ex in batch
                    ]
                else:
                    decoded_labels = [
                        self.tokenizer.decode(
                            ex["labels"], skip_special_tokens=True
                        ).replace(" ", "")
                        for ex in batch
                    ]
            ...
            for inputs, gold_outputs in tqdm(loader, desc="Evaluating (generation)"):
                ...
                if self.is_decoder_only:
                    for i in range(input_ids.shape[0]):
                        ...
                        texts = [
                            "[mech]".join(
                                self.tokenizer.decode(out).split("[mech]")[1:]
                            ).replace(" ", "")
                            for out in outputs
                        ]

                        texts = [
                            t.split("[eos]")[0] + "[eos]" if "eos" in t else t
                            for t in texts
                        ]
                    ...
                    decoded_preds = self.tokenizer.batch_decode(
                        [g_ids[1:] for g_ids in generated_ids]
                    )  # 1: to skip the default [pad] in t5
        ...
        if not generate_only:
                ...
                for j, text in enumerate(predictions):
                    try:
                        ...
                        result_row = {
                            ...
                            "at_least_two_mapped_atoms_used": res[
                                "at_least_two_mapped_atoms_used"
                            ],
                            "all_indices_used_were_mapped": res[
            ...
            folder_results = Path(
                "/scratch/neukomm/amlite/results"
            )  # Folder for batch results
            ...
            max_workers = (
                (cpus - 2) if (cpus := os.cpu_count()) is not None else 1
            )  # We let a few cpus resting on purpose (better perf. on my laptop)
            ...
            for n_b in range(n_batch_score):
                ...
                if os.path.exists(lock_file) or os.path.exists(result_file):
                    print(
                        f"Lock file or result file for batch {n_b} exists, skipping..."
                    )
                    continue
                ...
                try:
                    ...
                    print(
                        f"Batch {n_b} was just claimed by another worker, skipping..."
                    )
                    continue
            ...
            if parquet_file_path is None:
                for i, (input_text, gold_output) in tqdm(
                    enumerate(zip(inputs_list, references))
                ):
                    print(f"{Fore.LIGHTBLACK_EX}Input: {input_text}{Fore.RESET}")
                    print(
                        f"{Fore.LIGHTBLACK_EX} Gold: {Fore.LIGHTGREEN_EX}{gold_output}{Fore.RESET}"
                    )
                    for i, text in enumerate(predictions[i]):
                        print(
                            f"{Fore.LIGHTBLACK_EX}{i+1:>5.0f}: {Fore.LIGHTYELLOW_EX}{text}{Fore.RESET}"
                        )

                    all_gens = "".join(
                        [f"{n_beam+1:>4.0f} " for n_beam in range(num_beams)]
                    )
                    print(f"                                {all_gens}")
                    print("----- Score checks -----")
                    for metric in [
                        ...
                        all_scores = "".join(
                            [
                                f"[{answers_dict[results[-num_beams + i][metric]]}]"
                                for i in range(num_beams)
                            ]
                        )
                        print(
                            f"{metric.replace('_', ' ').capitalize():<32} {all_scores}"
                        )

                    print("----- Match GT ------")
                    for metric in [
                        "ground_truth_prod_sim",
                        "ground_truth_equiv",
                        "ground_truth_exact",
                    ]:
                        all_scores = "".join(
                        ...
                        print(
                            f"{Style.BRIGHT if metric == 'ground_truth_equiv' else Style.RESET_ALL}{metric.replace('_', ' ').capitalize():<32} {all_scores}{Style.RESET_ALL}"
                        )
            ...
            if parquet_file_path:
                ...
                test_dataset.to_parquet(
                    parquet_file_path.rsplit(".", 1)[0] + "_questions.parquet"
                )
    ...
    def tokenize_and_filter_encoder_decoder(self, example):
        if self.tokenizer is None:
            raise ValueError(
                "Tokenizer is None, please initialize it before running tokenize function"
            )
    ...
    def import_process_tokenize_dataset(
        ...
        for split in splits:
            ...
            if dataset_from_hub:
                if ";" in dataset_path:
                    dict_dataset_raw[split] = concatenate_datasets(
                        [
                            load_dataset(ds, split=split, token=os.getenv("HF_TOKEN"))
                            for ds in dataset_path.split(";")
                        ]
                    )
                else:
                    dict_dataset_raw[split] = load_dataset(
                        dataset_path, split=split, token=os.getenv("HF_TOKEN")
                    )
            else:
            ...
            dict_tokenized_dataset[split] = (
                dict_dataset[split]
                .map(
                    self.tokenize_and_filter_decoder
                    if self.is_decoder_only
                    else self.tokenize_and_filter_encoder_decoder,
            ...
            print(
                f"{Fore.YELLOW}Import time            : {time01-time00:>6.2f}s{Fore.RESET}"
            )
            print(
                f"{Fore.YELLOW}Shuffle time           : {time02-time01:>6.2f}s{Fore.RESET}"
            )
            print(
                f"{Fore.YELLOW}Process time (parallel): {time03-time02:>6.2f}s{Fore.RESET}"
            )
            print(
                f"{Fore.YELLOW}Filter time            : {time04-time03:>6.2f}s{Fore.RESET}"
            )

        if return_untok_dataset:
            return (
                (dict_tokenized_dataset[split], dict_dataset[split]) for split in splits
            )
        else:
