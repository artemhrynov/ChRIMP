...
def canonicalize_unmap_list(
    ...
    canonicalized_list = [
        canonicalize_unmap(smile, verbose=verbose) for smile in smiles_list
    ]
    if sort:
...
def evaluation_metrics_similarity(
    ...
    assert (
        math.floor(dampening_coeff / step_one_over_x) * step_one_over_x < 1
    ), f"Your dampening_coeff ({dampening_coeff}) is not low enough or your step_one_over_x ({step_one_over_x}) not high enough"

    if (
        smiles == ref_smiles
    ):  # Always true and might gain some time as string comparison take a negligible time
        return 1

    can_list_smiles = canonicalize_unmap_list(smiles, sort=False, verbose=verbose)
    can_list_ref_smiles = canonicalize_unmap_list(
        ref_smiles, sort=False, verbose=verbose
    )

    try:
        if (
            authorize_duplicate_species
        ):  # Set is the appropriate object here (We don't care about stoichiometry)
            ...
            if authorize_not_all_species:  # We check the subset
                if can_counter_smiles.keys() <= can_counter_ref_smiles.keys():
                    if all(
                        [
                            smiles_count <= can_counter_ref_smiles[smiles]
                            for smiles, smiles_count in can_counter_smiles.items()
                        ]
                    ):
                        return 1
...
def evaluation_metrics_reac(
    ...
    msmi_reac, msmi_arrows = (
        msmi_string.split("|", 1) if ("|" in msmi_string) else (msmi_string, "")
    )
...
def evaluation_metrics_indices(
    ...
    all_maps_used = (
        set(re.findall(r"\d+", search_result.group(1))) if search_result else set()
    )
...
def evaluation_metrics_wo_groundtruth(
    ...
    for mandatory in ["[reac]", "[mech]", "[eos]"] + (
        ["prod"] if format != "reac" else []
    ):
        if mandatory not in total_generation:
            if verbose:
                print(
                    f"Incorrect format of the generation {mandatory} not in {total_generation}"
                )
    ...
    if format == "retro":
        ...
        prod_string, reac_string, msmi_string = (
            match.group(1),
            match.group(2),
            match.group(3),
        )
        ...
        match = re.match(pattern_gen, total_generation)
        reac_string, prod_string, msmi_string = (
            match.group(1),
            match.group(2),
            match.group(3),
        )
    ...
    if msmi_reac_legal:
        (
            ...
            all_indices_used_were_mapped,
            every_index_unique,
        ) = evaluation_metrics_indices(msmi_string)

        mechsmiles_legal = (
            msmi_reac_legal and all_indices_used_were_mapped and every_index_unique
        )
        if mechsmiles_legal:
...
def evaluation_metrics_w_groundtruth(
    ...
    if metrics["msmi_reac_correct"] and metrics["product_stable"]:
        ...
        ground_truth_equiv = float(
            metrics["msmi_reac_correct"] == 1 and prod_ground_truth_prod_similarity == 1
        )
...
def evaluate_task_hf(
    ...
    if evaluate_on_train_set:
        print(
            f"{Fore.RED}Evaluating on train set for debug, don't forget to change that back{Fore.RESET}"
        )
    ...
    for inputs, gold_outputs in tqdm(loader, desc="Evaluating (generation)"):
        ...
        if decoder_only:
            for i in range(input_ids.shape[0]):
                ...
                texts = [
                    "[mech]".join(tokenizer.decode(out).split("[mech]")[1:]).replace(
                        " ", ""
                    )
                    for out in outputs
                ]
                ...
                texts = [
                    t.split("[eos]")[0] + "[eos]" if "eos" in t else t for t in texts
                ]
                predictions.append(texts)
                references.append(gold_outputs[i] + tokenizer.eos_token)
                inputs_list.append(
                    tokenizer.decode(input_ids[i]).replace(" ", "").split("[pad]")[-1]
    ...
    for i, (input_text, gold_output) in enumerate(zip(inputs_list, references)):
        tasks.append(
            (k, input_text, gold_output, predictions[i], (parquet_file_path is None))
        )
        k += 1
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        ...
        for fut in tqdm(
            as_completed(futures), total=total, desc="Evaluating (scoring)"
        ):
            try:
