...
class MechSmiles:
    ...
    def init_everything_from_value(self, value, conds=None, context=None):
        ...
        if context is not None:
            ...
            already_acounted_species_value = Counter([quick_canonicalize(x) for x in self.ms.can_smiles.split('.')])
            already_acounted_species_cond = Counter([quick_canonicalize(c) for c in self.conds])
            all_context = Counter([quick_canonicalize(x) for x in context.split('.')])

            if not set(already_acounted_species_value.keys()).issubset(set(all_context.keys())):
                #print(f"{set(already_acounted_species_value.keys())=}")
                #print(f"{set(all_context.keys())=}")
                raise MechSmilesContextError(f"Context incorrect, doesn't contain the reacting species\n{already_acounted_species_value=}\n{all_context=}")

            difference = all_context - already_acounted_species_value - already_acounted_species_cond
    ...
    def hide_cond(self, unmap_non_reactive_atoms=True):
        ...
        self.init_everything_from_value(f"{new_smiles}|{self.all_arrows_string}", conds=[s for s in species if not regex.search(s)])

    def unhide_cond(self):
        new_smiles = '.'.join(([self.smiles] if self.smiles != "" else []) + self.conds)
        self.init_everything_from_value(f"{new_smiles}|{self.all_arrows_string}", conds=[])
    ...
    def kekulize(self):
        ...
        Chem.SanitizeMol(new_mol, Chem.SanitizeFlags.SANITIZE_ALL ^ Chem.SanitizeFlags.SANITIZE_SETAROMATICITY)
        mapped_or_isolated_idx = [a.GetIdx() for a in new_mol.GetAtoms() if (a.GetSymbol() == 'H' and (a.GetAtomMapNum() > 0 or all([b.GetOtherAtom(a).GetSymbol() == 'H' for b in a.GetBonds()])))]
        new_new_mol, _ = filter_hydrogens(new_mol, [], mapped_or_isolated_idx)

        self.init_everything_from_value(Chem.MolToSmiles(new_new_mol) + '|' + self.all_arrows_string)
    ...
    def unmap_nonreactive_atoms(self):
        ...
        for arrow in self.smiles_arrows:
            reactive_indices_arrow = [part.strip() for part in re.split(r'[(),]', arrow) if part]
    ...
    def minimize_indices(self):
        ...
        max_reactive_index = max([int(x) for x in reactive_indices]) if len(reactive_indices) > 0 else 0
        dict_remap_1 = {str(idx): str(max_reactive_index+i+1) for i, idx in enumerate(reactive_indices)}
        dict_remap_2 = {str(max_reactive_index+i+1): str(i+1) for i, _ in enumerate(reactive_indices)}
        ...
        tmp_smiles_arrows = [str(remap_tuple(eval(tup_str), dict_remap_1)) for tup_str in self.smiles_arrows]
        for old_idx, new_idx in dict_remap_1.items():
            final_smiles = re.sub(f':{old_idx}]', f':{new_idx}]', final_smiles)

        all_arrows_string = ';'.join(str(x) for x in [remap_tuple(eval(tup_str), dict_remap_2) for tup_str in tmp_smiles_arrows])
    ...
    def check_validity(self):
        ...
        if not MechSmiles.warning_dummy_already_printed:
            print(f"{Fore.YELLOW}Careful, validity not implemented yet, this is a dummy test{Fore.RESET}")
    ...
    def prod(self):
        if self._prod is None:
            if self.check_validity():
                self._ms_prod = self.ms.make_move([self.process_smiles_arrow(a, self.ms.atom_map_dict) for a in self.smiles_arrows])
    ...
    def process_smiles_arrow(self, arrow_smiles, atom_map_dict):
        ...
        if isinstance(arrow_smiles, str):
            ...
            raise ValueError(f"Arrow must be a string, instead is: {type(arrow_smiles)} with value {arrow_smiles}")
        ...
        if isinstance(tup[0],int): # attack move
            ...
            if not len(tup[0])==2:
                raise ValueError("Move must either have a format (a, b), ((a, b), c) or (hv, (a, b))")

            if tup[0][1] == tup[1]:
                return ('i', atom_map_dict[tup[0][0]], atom_map_dict[tup[0][1]])

            else:
                return ('ba',atom_map_dict[tup[0][0]], atom_map_dict[tup[0][1]], atom_map_dict[tup[1]])
    ...
    def drawn_arrows_to_mechSmiles(cls, smiles, drawn_arrows):
        ...
        for arrow in drawn_arrows:
            ...
            assert isinstance(arrow[0], tuple) or isinstance(arrow[0], int), "elements of drawn_arrows must be either a tuple or an int"
            assert isinstance(arrow[1], tuple) or isinstance(arrow[1], int), "elements of drawn_arrows must be either a tuple or an int"
        ...
        for drawn_arrow in drawn_arrows:
            ...
            if isinstance(drawn_arrow[0], int) and isinstance(drawn_arrow[1], int):
                ...
                if last_atom_attacked is None:
                    ...
                    raise NotInitiatingAttackError(f"The drawn arrow {drawn_arrow} for SMILES {smiles} is not valid, an attack move has to initiate")
            ...
            # ((a1, a2), a3)
                ...
                # If a1 == a3, same but we reverse the order in the bond
                    ...
                    final_arrows.append(((drawn_arrow[0][1], drawn_arrow[0][0]), drawn_arrow[1]))

                # If None of the above but the last attacked is in the bond:
                elif last_atom_attacked is not None and last_atom_attacked in drawn_arrow[0]:
                    other_atom = drawn_arrow[0][0] if last_atom_attacked == drawn_arrow[0][1] else drawn_arrow[0][1]
                    final_arrows.append(((last_atom_attacked, other_atom), drawn_arrow[1]))
            ...
            # ((a1, a2), (a3, a4))
            elif isinstance(drawn_arrow[0], tuple) and isinstance(drawn_arrow[1], tuple):
                # Find overlapping atoms
                intersection_list = list(set(drawn_arrow[0]).intersection(set(drawn_arrow[1])))
                assert len(intersection_list) == 1, f"The drawn arrow must have exactly one common atom, arrow {drawn_arrow} for SMILES {smiles} has {len(intersection_list)} common atoms"
                common_atom = intersection_list[0]
                other_atom_1 = drawn_arrow[0][0] if common_atom == drawn_arrow[0][1] else drawn_arrow[0][1]
                other_atom_2 = drawn_arrow[1][0] if common_atom == drawn_arrow[1][1] else drawn_arrow[1][1]
            ...
            # (a, (b, c))
                ...
                assert drawn_arrow[0] in drawn_arrow[1], f"AmbiguousError: An atom can only attack a bond if it is adjacent to it, drawn arrow {drawn_arrow} for SMILES {smiles} is not valid"
                ...
                if drawn_arrow[0] == drawn_arrow[1][0]:
                    ...
                    raise AssertionError(f"An atom can only attack a bond if it is adjacent to it, drawn arrow {drawn_arrow} for SMILES {smiles} is not valid")

            else:
                # Illegal (too ambiguous)
                raise ValueError(f"The drawn arrow {drawn_arrow} for SMILES {smiles} is not valid, because it is ambiguous")
        ...
        print(f"before filtering for mapping, smiles is {Chem.MolToSmiles(mol, kekuleSmiles=True)}, arrows are {final_arrows}, all_used_indices are {all_used_indices}")
        for atom in mol.GetAtoms():
