def aggregate_top_k(
    ...
    assert (
        filtering_strategy in strategy_list
    ), f"Filtering strategy can only be one of {strategy_list}"
    ...
    ):  # Here, we keep the predictions that have a beam-index of exactly k-1
        df_top_k = df[df["beam_index"] == (k - 1)].copy()

    elif (
        filtering_strategy == "keep_below_k"
    ):  # Here, we keep the predictions that have a beam-index below k
