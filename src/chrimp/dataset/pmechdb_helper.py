...
def translate_and_validate(pmechdb_string):
    ...
    pmechdb_string = re.sub(re.escape("[HH]"), "[H][H]", pmechdb_string) # For some reason H2 written as [HH] is not canonicalized like [H][H]
    pmechdb_string = re.sub(re.escape("N(=O)=O"), "[N+](=O)[O-]", pmechdb_string) # Rewrite nitro in a form that doesn't violate the octet
    new_pmechdb_string = re.sub(r'[@\\/]', '', pmechdb_string) # ChRIMP doesn't support chirality yet
    ...
    reac_pmdb, cond_pmdb, prod_pmdb = rxn_pmdb.split('>') # I will not use conditions for now
    ...
    for arr in arrows_pmdb:
        ...
        atck_entity, acpt_entity = arr.split('=') # Entity because we don't know yet if it is an atom or a bond

        atck_entity = ','.join([x for x in atck_entity.split(',') if x != '']) # Sanitize moves like 10=20, that is multiple times in the dataset
        ...
        if acpt_entity_is_bond:
            ...
            set_common_atoms = set(atck_entity.split(',')).intersection(set(acpt_entity.split(',')))
            assert len(set_common_atoms) == 1, "Didn't plan that yet"
            acpt_entity_split = acpt_entity.split(',')
            acpt_entity = acpt_entity_split[0] if acpt_entity_split[0] not in set_common_atoms else acpt_entity_split[1]
            ...
            if atck_entity_is_bond:
                ...
                atck_entity_split = atck_entity_split if atck_entity_split[0] not in set_common_atoms else atck_entity_split[::-1]
        ...
        if not atck_entity_is_bond: # atom to atom attack
            ...
            if acpt in atck_entity_split: # ionization
                atck = atck_entity_split[0] if atck_entity_split[0] != acpt else atck_entity_split[1]
    ...
    if len(source_atoms-sink_atoms) ==1 and len(sink_atoms-source_atoms)==1: # Unique source and sink, linear arrow flow
    ...
    elif len(source_atoms-sink_atoms) ==0 and len(sink_atoms-source_atoms)==0: # No source that is not also a sink, circular arrow flow
        ...
        print(f"{sink_atoms=}")
        print(f"I don't understand the arrow-flow of {pmechdb_string}\n{source_to_arrow_sink=}")
    ...
    similar_prod = remove_mapping_and_canonicalize(prod_pmdb, addHs=True) == remove_mapping_and_canonicalize(mech_smi.prod, addHs=True)
