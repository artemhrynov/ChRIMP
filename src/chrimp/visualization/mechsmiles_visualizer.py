...
class MechSmilesVisualizer:
    ...
    def show_reac(self, msmi, save_path=None, return_svg=False, hv_icon=None):
        ...
        arrow_list = [msmi.process_smiles_arrow(a, ms.atom_map_dict) for a in msmi.smiles_arrows]
        ms = MoleculeSet.from_smiles(msmi.smiles)

        return ms.show_move_svg(arrow_list, save_path=save_path, return_svg=return_svg, hv_icon=hv_icon)

    def show_prod(self, msmi, save_path=None, return_svg=False, hv_icon=None):
        return msmi.ms_prod.show_move_svg([], save_path=save_path, return_svg=return_svg, hv_icon=hv_icon)

    def show_cond(self, msmi, save_path=None, return_svg=False, hv_icon=None):
        conds = msmi.conds
        return MoleculeSet.from_smiles('.'.join(x for x in conds)).show_move_svg([], save_path=save_path, return_svg=return_svg, hv_icon=hv_icon)
    ...
    def show(self, msmi, save_path=None, return_svg=False, hv_icon=False):
        ...
        if len(value_split) == 2:
            ...
            reac = self.show_reac(msmi, save_path=save_path, return_svg=True, hv_icon=hv_icon)
            prod = self.show_prod(msmi, save_path=save_path, return_svg=True, hv_icon=False)
            cond = self.show_cond(msmi, save_path=save_path, return_svg=True) if len(msmi.conds) > 0 else None
            ...
            raise NotImplementedError("Multiple mechanistic steps are not implemented yet")
            ...
            total_height = n_rows * pane_height * (1+vertical_space) - vertical_space * pane_height

import svgutils.transform as sg
from xml.etree import ElementTree as ET

def reaction_svg(reac_svg:str, prod_svg:str, cond_svg:Union[str, None] = None,
                 arrow_len:float = 80,    # px
                 arrow_thickness:int = 2) -> str:
    """
...
def make_arrow_svg(arrow_len: float,
                   h_max: float,
                   arrow_thickness: float = 1.6,
                   head_factor: float = 3.0) -> str:
    """
